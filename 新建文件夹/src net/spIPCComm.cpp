...
int CIPCSocket::RecvWithTimeOut(int iTimeout,const int iWant,BYTE* pbBuf,int& iRead)
{
	...
	while(1)
	{
		...
		if(SP_ERR_SUCCESS == iTest)
		{
			...
			{
				OutputDebugString(_T("Socket Error = SP_EER_NETWORK"));
				return SP_ERR_NETWORK;
			}
			iRecved += iRet;
			iRead = iRecved;
		...
		{
			//OutputDebugString(_T("Socket Error = SP_ERR_NETWORK"));
			return SP_ERR_NETWORK;
		}
		...
		{//检查是否超时
			if (iRead ==0)
			{
				OutputDebugString(_T("Socket Error = SP_ERR_NODATA"));
				return SP_ERR_NODATA;
			}
			else
			{
				OutputDebugString(_T("Socket Error = SP_ERR_NOT_FINISH"));
				return SP_ERR_NOT_FINISH;
			}
		}
	}

	OutputDebugString(_T("Socket Error = SP_ERR_GENERAL"));
	return SP_ERR_GENERAL;
}
