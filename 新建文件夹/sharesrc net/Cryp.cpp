...
#define FILE_SIZE_READ_BUFF (1024 * 1024)
int CCryp::GetFileMD5(LPTSTR strFilePath, unsigned char digest[16])
{
	...
	DWORD dwRead = 0;
	BYTE *pBuff = new BYTE[FILE_SIZE_READ_BUFF];
	while(dwLength >= FILE_SIZE_READ_BUFF)
	{
		ReadFile(hFile, pBuff, FILE_SIZE_READ_BUFF, &dwRead, NULL);
		MD5Update(pBuff, FILE_SIZE_READ_BUFF);
		dwLength = dwLength - dwRead;
	}
	if (dwLength)
	{
		ReadFile(hFile, pBuff, FILE_SIZE_READ_BUFF, &dwRead, NULL);
		MD5Update(pBuff, dwRead);
		dwLength = dwLength - dwRead;
	}
...
int CCryp::FileToMD5(LPTSTR strFilePath, LPBYTE out)
{
	HANDLE hFile = CreateFile(strFilePath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (hFile == NULL || hFile == INVALID_HANDLE_VALUE)
	{
...
U32 CCryp::FromBase30( char *out )
{
	...
	int		index,
			count = (strchr( table, out[7] ) - table) / 2 +1;
	
	for( index = 0; index < count; index++ )
	{
		an[index] = strchr( table, out[index] ) - table;
	}
