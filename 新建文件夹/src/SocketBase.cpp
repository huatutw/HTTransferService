...
void  CSocketBase::SendStringData(CIPCSocket& s,LPTSTR strContent,int nSize)
{
	int iWrote = 0;
	int dwSize = nSize;
	s.Send(sizeof(int),(LPBYTE)&dwSize,iWrote);

	if (nSize != 0)
		s.Send(nSize*sizeof(TCHAR),(LPBYTE)strContent,iWrote);
}

void  CSocketBase::SendStringData(CIPCSocket& s,LPCWSTR strContent)
{
	int iWrote = 0;
	int nSize = 0;
	if (strContent == NULL)
	...
	if (nSize != 0)
		s.Send(nSize*sizeof(WCHAR),(LPBYTE)strContent,iWrote);
}

void  CSocketBase::SendStringData(CIPCSocket& s,LPCSTR strContent)
{
	int iWrote = 0;
	int nSize = 0;
	if (strContent == NULL)
	...
	if (nSize != 0)
		s.Send(nSize*sizeof(CHAR),(LPBYTE)strContent,iWrote);
}
...
void  CSocketBase::SendData(CIPCSocket& s,LPVOID lpData,int nSize)
{
	int iWrote;
...
void  CSocketBase::SendFile1(CIPCSocket &s,LPCSTR lpszFileName)
{
	...
	LPSTR strFileName = strrchr(lpszFileName,'\\');
	if (strFileName)
	{
		strFileName ++;
	}
...
void  CSocketBase::SendFile1(CIPCSocket &s,LPCWSTR lpszFileName)
{
	...
	LPWSTR strFileName = wcsrchr(lpszFileName,'\\');
	if (strFileName)
	{
		strFileName ++;
	}
