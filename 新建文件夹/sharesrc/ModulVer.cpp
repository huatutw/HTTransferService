...
BOOL CModuleVersion::GetFileVersionInfoEx(LPTSTR modulename)
{
	m_translation.charset = 1252;		// default = ANSI code page
	memset((VS_FIXEDFILEINFO*)this, 0, sizeof(VS_FIXEDFILEINFO));
	...
	// read file version info
	DWORD dwDummyHandle; // will always be set to zero
	int len = GetFileVersionInfoSize(modulename, &dwDummyHandle);
	if (len <= 0)
		return FALSE;
...
//
void CModuleVersion::GetValue(LPCTSTR lpKeyName,LPTSTR lpValue)
{
	if (m_pVersionInfo) {
		...
		//
		TCHAR query[1024] = {0};
		_stprintf(query,_T("\\StringFileInfo\\%04x%04x\\%s"),
