...
int DGCommonFunc::Base64Decode(const char *pszIn, int iInSize, char *pOut, int *piOutSize)
{
...
#ifndef DGCLIENT_SETUP
BOOL DGCommonFunc::ReadRegDataEx(LPTSTR strUser,LPTSTR strPass)
{
	CRegKey DocKey;
	if (DocKey.Open(HKEY_CURRENT_USER,TEXT("software\\{F4C09C2D-E206-75B7-2DBF-15DBC6957210}\\Client"),KEY_READ) != ERROR_SUCCESS)
	{
		if (DocKey.Open(HKEY_CURRENT_USER,TEXT("software\\DocGuarder\\Client"),KEY_READ) != ERROR_SUCCESS)
		{
			return FALSE;
...
{
	CRegKey DocKey;
	if (DocKey.Open(HKEY_LOCAL_MACHINE,TEXT("software\\{F4C09C2D-E206-75B7-2DBF-15DBC6957210}\\Connect"),KEY_READ) != ERROR_SUCCESS)
	{
		if (DocKey.Open(HKEY_LOCAL_MACHINE,TEXT("software\\DocGuarder\\Connect"),KEY_READ) != ERROR_SUCCESS)
		{
			return FALSE;
...
BOOL DGCommonFunc::ReadNamServerData(LPTSTR strNamServerIP, DWORD& dwTcpServerPort, DWORD& dwUdpServerPort)
{
	CRegKey DocKey;
	if (DocKey.Open(HKEY_LOCAL_MACHINE,TEXT("software\\{F4C09C2D-E206-75B7-2DBF-15DBC6957210}\\Connect"),KEY_READ) != ERROR_SUCCESS)
	{
	...
	DWORD dwLen = 1024;
	DocKey.QueryValue(strNamServerIP,TEXT("Samserverip"),&dwLen);

	DocKey.QueryValue(dwTcpServerPort,TEXT("tcpport"));

	DocKey.QueryValue(dwUdpServerPort,TEXT("udpport"));
...
{
	CRegKey DocKey;
	if (DocKey.Open(HKEY_LOCAL_MACHINE,TEXT("software\\{F4C09C2D-E206-75B7-2DBF-15DBC6957210}"),KEY_WRITE) != ERROR_SUCCESS)
	{
		DocKey.Create(HKEY_LOCAL_MACHINE,TEXT("software\\{F4C09C2D-E206-75B7-2DBF-15DBC6957210}"),REG_NONE,REG_OPTION_NON_VOLATILE,KEY_WRITE);
	}else
	{
		DocKey.Close();
	}

	if (DocKey.Open(HKEY_LOCAL_MACHINE,TEXT("software\\{F4C09C2D-E206-75B7-2DBF-15DBC6957210}\\Server"),KEY_WRITE) != ERROR_SUCCESS)
	{
		DocKey.Create(HKEY_LOCAL_MACHINE,TEXT("software\\{F4C09C2D-E206-75B7-2DBF-15DBC6957210}\\Server"),REG_NONE,REG_OPTION_NON_VOLATILE,KEY_WRITE);
		DocKey.Open(HKEY_LOCAL_MACHINE,TEXT("software\\{F4C09C2D-E206-75B7-2DBF-15DBC6957210}\\Server"),KEY_WRITE);	
	}
...
// 		m_MacAddr[0].b5,m_MacAddr[0].b6);
	...
	OLECHAR id[1024];
	StringFromGUID2(guid,id,1024);
	
	USES_CONVERSION;
	strMac = OLE2T(id);

	strMac = strMac.Left(32);
...
BOOL DGCommonFunc::GetMacByCmd( CAString &strMac )
{
	...
	//网卡MAC地址的前导信息
	const CAString str4Search = _T("Physical Address. . . . . . . . . : ");
...
/*	char szSystemInfo[1024] = {0};
	...
	CString strHardID = szSystemInfo;
	...
	char cout[512] = {0};
	ToBase64(out, strlen((char*)out), cout);
	strSerialOut = cout;
...
#ifndef DGCLIENT_SETUP
CAString DGCommonFunc::EncryptRC4Base64(CAString strSource)
{
	USES_CONVERSION;
	char ckey[BUFF_SIZE] = {0};
	memcpy(ckey, T2A(strSource), strlen(T2A(strSource)));
	int nLen = strlen(ckey);
	DGCommonFunc::Encrypt_RC4((LPBYTE)ckey, nLen, "DocGuarder_Encrypt_System_Made_in_HangZhou_2011_10_26");
	char out[BUFF_SIZE] = {0};
	DGCommonFunc::ToBase64((const unsigned char*)ckey, nLen, out);
...
CAString DGCommonFunc::DecryptRC4Base64(CAString strSource)
{
	USES_CONVERSION;
	char out[BUFF_SIZE] = {0};
	int nOut = BUFF_SIZE;
	//DGCommonFunc::FromBase64((const unsigned char*)T2A(strSource),strlen(T2A(strSource)),out);
	DGCommonFunc::Base64Decode(T2A(strSource),strlen(T2A(strSource)), out, &nOut);
	DGCommonFunc::Encrypt_RC4((LPBYTE)out, nOut, "DocGuarder_Encrypt_System_Made_in_HangZhou_2011_10_26");
