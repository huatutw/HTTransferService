...
//	以下三个函数用于将字符串转换为规定字符集合的字符串
	...
	static int  Base64Decode(const char *pszIn, int iInSize, char *pOut, int *piOutSize);
////////////////////////////////////////////////////////////////////////////////////////////
//  以下为rc4算法的函数
...
//  将string转换为IP, 127.0.0.1 -> 127 0 0 1
	...
	static CAString EncryptRC4Base64(CAString strSource);
	static CAString DecryptRC4Base64(CAString strSource);
...
//  读取注册表的数据
	//Ex为UNICODE读取方式
	static BOOL ReadRegDataEx(LPTSTR strUser,LPTSTR strPass);
...
//	static BOOL ReadRegEGetWayData(LPTSTR strEGetWayIP, DWORD& dwEGetWayPort);

	static BOOL WriteRegServerData(DWORD dwServerPort);
	static BOOL WriteRegUpdateData(DWORD dwUpdatePort);

	static BOOL ReadNamServerData(LPTSTR strNamServerIP, DWORD& dwTcpServerPort, DWORD& dwUdpServerPort);
//  从服务器验证用户,并得到当前用户的权限
